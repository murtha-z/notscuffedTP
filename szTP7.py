...
# Date: 6 December
# Title: Sandbox game (more of a snowball game tbh)
# Approx Line Count: --
#################################################
...
Big todo list:
    citations
    style (lines then OOP)
'''
print("\n*************PRINT STATEMENTS:*************")

#MAJOR BUGS WITH FILES 

#==============================================================================
#==============================================================================
# Main Menu / Splash Page
#==============================================================================
#==============================================================================
...
class startScreen(Mode):
    ...
    uiText = {  "aboveTitle": "._. 15112 f20 TP by shelly ._.", 
    ...
    infoText = '''
    ex. Press 'm' to return to this menu from other screens
    honestly, this is more like a snowball dropper than anything else
    '''
    #THE MOST IMPORTANT THING:
    #THE PATH TO THE FILE WE WRITE/READ AND TAG FOR TYPE OF FILE
    thePath = None
    oldOrNew = None
    def appStarted(mode):
        # sizing, dynamic
        ...
        mode.firstLine = None #what is this
    ...
    def makeAndOpenFile(mode, path):
        with open(path, 'w+') as mode.fileOpen:
            ...
            #sample = {"mode":"CreativeSandbox","name":"filler","listOfData":[True]}
            #mode.fileOpen.write(str(sample))
    ...
    def profileMousePressed(mode, x, y):
        ...
        if (0 <= x <= startScreen.goBack[2]) and (0 <= y <= startScreen.goBack[3]):
        ...
        elif (topx <= x <= botx) and (topy <= y <= boty):
            topx = mode.width * .1
            topy = mode.height * .1
            botx = mode.width * .9
            boty = mode.height * .9
            ...
            #GO TO OLD PROFILE MENU
            if startScreen.thePath != "Should Do Nothing": #is this janky, oh god
                mode.app.setActiveMode(mode.app.oldProjectMenu)
    ...
    def toggleInfo(mode, canvas):
        ...
        canvas.create_text(mode.width // 2, mode.height // 3, text = "INFO - press 'i'", font = "Times 28 bold", fill = "white")
        canvas.create_text(mode.width // 2, mode.height // 2, text = startScreen.infoText, font = "Times 20 bold", fill = "white")
            ...
            for row in range(rows):
                ...
                if len(listOfProfiles) > listTag: #TODO: remove the stupid DS and .txt one
                    if listOfProfiles[listTag] != ".txt" or listOfProfiles[listTag] != ".DS_Store":
                        ...
                        #font adjustment, not perfect but better and adjustable :}
                        fontSize = int(colWidth // len(listOfProfiles[listTag]))
                        text = str(listOfProfiles[listTag])
                        if fontSize < 10:
                            fontSize = 15
                            halfway = int(len(listOfProfiles[listTag]) // 2)
                            first, second = text[:halfway], text[halfway:]
                            text = f"{first}-\n{second}"
                        elif fontSize >= rowHeight:
                            fontSize = int(rowHeight * .9)
                        if len(listOfProfiles) < 5:     
                            fontSize = rowHeight//2
                        font = f"Times {int(fontSize)} bold"
                        canvas.create_text(x1 + (.5*colWidth),y1 + (.5*rowHeight), text = text, fill = "grey", font = font)
                        #canvas.create_text(x1 + (.5*colWidth),y1 + (.5*rowHeight), text = textDisplay, fill = startScreen.uiText["buttonCol"], font = font)
                    listTag += 1
                y1 += rowHeight
                y2 += rowHeight
    ...
    def toggleProfileExistsMessage(mode, canvas):
        ...
        canvas.create_text( (botx+topx)//2, (boty+topy)//2, text = "This Profile Already Exists... or You Clicked 'Cancel'\n\n:3\n\n*either way*\n\nCLICK ANYWHERE TO GET BACK TO BUSINESS", fill = startScreen.uiText["textCol"], font = mode.buttonFont, anchor = "center")
...
# Menu pour new Profile
class menuNew(Mode):
    goBack = ["Back to Menu", "grey", 100, 60, "Times"]
    uiText = {  "sbFUN": "New Creative Sandbox",
                "sbPLAY": "Play Levels",
    ...
    #TODO: elif buttons: display the togglable modes
    #see other mode : ]
    def mousePressed(mode,event):
        x,y = event.x, event.y
        ...
        #button2: PLAY LEVELS
            ...
            mode.app.setActiveMode(mode.app.sbPLAY) #BROKEY
            ...
            mode.app.setActiveMode(mode.app.sbCREATE) #BROKEY
    ...
    def redrawAll(mode, canvas):
        ...
        #Title
        canvas.create_text(width4th*2, (height4th*.5), font = mode.titleFont, text = f'Menu for New User: "{startScreen.thePath}"')
...
class menuProfiles(Mode):
    goBack = ["Back to Profiles", "grey", 100, 60, "Times"]
    uiText = {  "sbFUN": "Creative Sandbox",
                "sbPLAY": "Play Levels",
                "sbCREATE": "Create Levels",
                "textCol":"white",
                "buttonCol":"grey", }
    sbFUN, sbPLAY, sbCREATE = False, False, False

    def appStarted(mode):
        #mode type tag
        startScreen.oldOrNew = "old"
        #"standard" font here
        mode.font = min(mode.width, mode.height) // 20
        mode.titleFont = f"Times {int(min(mode.width, mode.height)//20)} bold"
        #button margins
        ...
        mode.buttonFont = f"Times {int(min(mode.width, mode.height)//30)} bold" #repeat
        #FILE stuff
        menuProfiles.parseTextFile(mode)
    ...
    def mousePressed(mode,event):
        ...
        if (0 <= x <= menuProfiles.goBack[2]) and (0 <= y <= menuProfiles.goBack[3]):
            mode.app.setActiveMode(mode.app.mainMenuMode)
        #toggles the pop-up off
        if menuProfiles.sbFUN or menuProfiles.sbPLAY or menuProfiles.sbCREATE:
            if (0 <= x <= startScreen.goBack[2]) and (0 <= y <= startScreen.goBack[3]):
                menuProfiles.sbFUN, menuProfiles.sbPLAY, menuProfiles.sbCREATE = False, False, False
        ...
        #button2: PLAY LEVELS
            ...
            menuProfiles.sbPLAY = True
            #CLICK
            #mode.app.setActiveMode(mode.app.sbPLAY)
        
        elif (width4th <= x <= width4th*3) and (height4th*3 + heightMargin <= y <= height4th*4 - heightMargin):
        
            menuProfiles.sbCREATE = True
            #CLICK
            #mode.app.setActiveMode(mode.app.sbCREATE)


    #FILE STUFF
    #TODO: STRING OF 3D LIST OF POINTS NEEDS TO BE A STRING
    def parseTextFile(mode):
        mode.listFUN = []
        mode.listPLAY = []
        mode.listCREATE = []
        #READ ONLY HERE
        with open(f"Profiles/{startScreen.thePath}", 'r') as openedFile:
            #title = openedFile.name
            #print(repr(title))
            for line in openedFile:
                listOfData = line.split(";")
                if listOfData[0] == "CreativeSandbox":
                    #MAKE LIST OF DICTS FOR EACH TYPE OF MODE
                    incomingDict = dict()
                    incomingDict["title"] = listOfData[1].replace('\n','')
                    incomingDict["data"] = listOfData[2].replace('\n','') #TODO: STRING TO LIST
                    mode.listFUN.append(incomingDict)
                elif listOfData[0] == "PlayLevelSandbox":
                    #MAKE LIST OF DICTS FOR EACH TYPE OF MODE
                    incomingDict = dict()
                    incomingDict["title"] = listOfData[1].replace('\n','')
                    incomingDict["data"] = listOfData[2].replace('\n','') #TODO: STRING TO LIST
                    mode.listPLAY.append(incomingDict)
                elif listOfData[0] == "LevelCreateSandbox":
                    #MAKE LIST OF DICTS FOR EACH TYPE OF MODE
                    incomingDict = dict()
                    incomingDict["title"] = listOfData[1].replace('\n','')
                    incomingDict["data"] = listOfData[2].replace('\n','') #TODO: STRING TO LIST
                    mode.listCREATE.append(incomingDict)
    ...
    def mainMenuUI(mode, canvas):
        ...
        #Title
        canvas.create_text(width4th*2, (height4th*.5), font = mode.titleFont, text = f'Menu for Old User: "{startScreen.thePath}"')
        #Button UI  #canvas.create_rectangle(width4th, height4th*0 + heightMargin, width4th*3, height4th*1 - heightMargin)
        canvas.create_rectangle(width4th, height4th*1 + heightMargin, width4th*3, height4th*2 - heightMargin, fill = menuProfiles.uiText["buttonCol"], outline = menuProfiles.uiText["buttonCol"])
        canvas.create_rectangle(width4th, height4th*2 + heightMargin, width4th*3, height4th*3 - heightMargin, fill = menuProfiles.uiText["buttonCol"], outline = menuProfiles.uiText["buttonCol"])
        canvas.create_rectangle(width4th, height4th*3 + heightMargin, width4th*3, height4th*4 - heightMargin, fill = menuProfiles.uiText["buttonCol"], outline = menuProfiles.uiText["buttonCol"])
        #ButtonText UI
        canvas.create_text(width4th*2, (height4th*1.5), font = mode.buttonFont, fill = menuProfiles.uiText["textCol"], text = menuProfiles.uiText["sbFUN"])
        canvas.create_text(width4th*2, (height4th*2.5), font = mode.buttonFont, fill = menuProfiles.uiText["textCol"], text = menuProfiles.uiText["sbPLAY"])
        canvas.create_text(width4th*2, (height4th*3.5), font = mode.buttonFont, fill = menuProfiles.uiText["textCol"], text = menuProfiles.uiText["sbCREATE"])
    ...
    #FILES UI POP UP THINGS
    def redrawFUN(mode, canvas, width4th, height4th):
        canvas.create_rectangle(0,0,mode.width,mode.height, fill = "black")
        numberOfFiles = len(mode.listFUN)
        if numberOfFiles <= 1: numberOfFiles = 1
        rows = 10
        cols = math.ceil(numberOfFiles / rows)
        rowHeight = mode.height //rows
        colWidth = mode.width //cols
        x1 = 0
        y1 = 0
        x2 = colWidth
        y2 = rowHeight
        ...
        for col in range(cols):
            for row in range(rows):
                ...
                if len(mode.listFUN) > listTag:
                    canvas.create_text(x1+(.5*colWidth), y1+(.5*rowHeight), text = mode.listFUN[listTag]["title"], fill = "grey") #,font = )
                listTag += 1
                y1 += rowHeight
                y2 += rowHeight
            y1, y2 = 0, rowHeight
        ...
        canvas.create_text(startScreen.goBack[2]//2, startScreen.goBack[3]//2, text = startScreen.goBack[0], font = startScreen.goBack[4])
    #TODO: CHEKCk this
    def redrawPLAY(mode, canvas, width4th, height4th):
        canvas.create_rectangle(0,0,mode.width,mode.height, fill = "black")
        numberOfFiles = len(mode.listPLAY)
        if numberOfFiles <= 1: numberOfFiles = 1
        rows = 10
        cols = math.ceil(numberOfFiles / rows)
        rowHeight = mode.height //rows
        ...
        for col in range(cols):
            for row in range(rows):
                canvas.create_rectangle(x1,y1,x2,y2,outline = "white")
                if len(mode.listPLAY) > listTag:
                    canvas.create_text(x1+(.5*colWidth), y1+(.5*rowHeight), text = mode.listPLAY[listTag]["title"], fill = "grey") #,font = )
                listTag += 1
                y1 += rowHeight
                y2 += rowHeight
            y1, y2 = 0, rowHeight
        ...
        canvas.create_text(startScreen.goBack[2]//2, startScreen.goBack[3]//2, text = startScreen.goBack[0], font = startScreen.goBack[4])
    #TODO: CHEKCk this
    def redrawCREATE(mode, canvas, width4th, height4th):
        canvas.create_rectangle(0,0,mode.width,mode.height, fill = "black")
        numberOfFiles = len(mode.listCREATE)
        if numberOfFiles <= 1: numberOfFiles = 1
        rows = 10
        cols = math.ceil(numberOfFiles / rows)
        rowHeight = mode.height //rows
        ...
        for col in range(cols):
            for row in range(rows):
                canvas.create_rectangle(x1,y1,x2,y2,outline = "white")
                if len(mode.listCREATE) > listTag:
                    canvas.create_text(x1+(.5*colWidth), y1+(.5*rowHeight), text = mode.listCREATE[listTag]["title"], fill = "grey") #,font = )
                listTag += 1
                y1 += rowHeight
                y2 += rowHeight
            y1, y2 = 0, rowHeight
    ...
    def redrawAll(mode, canvas):
        ...
        #Main Menu UI
        menuProfiles.mainMenuUI(mode, canvas)

        #DISPLAYS THE FILE CONTENTS
        if menuProfiles.sbFUN:
            menuProfiles.redrawFUN(mode, canvas, width4th, height4th)
        if menuProfiles.sbPLAY:
            menuProfiles.redrawPLAY(mode, canvas, width4th, height4th)
        if menuProfiles.sbCREATE :
            menuProfiles.redrawCREATE(mode, canvas, width4th, height4th)
...
class playLevelSandbox(SandboxingTime):
    #if new, add the dictionaries into the file
    #else, load up old ones
        #start game
    pass
